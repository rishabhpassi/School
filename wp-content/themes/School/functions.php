...
function fwd_excerpt_more1($more1){

	if(!is_post_type_archive( 'fwd-Student-category')){
		$more1 = '... <a class="read-more" href="' . esc_url( get_permalink() ) . '">Read More about the Student</a>';

		return $more1;
	}
}
add_filter( 'excerpt_more1', 'fwd_excerpt_more1' );



// Block Template Code for pages
 ...
 * Enqueue scripts and styles.
 */
function fwd_add_google_fonts() { wp_enqueue_style( 'fwd-google-fonts', 'https://fonts.googleapis.com/css2?family=Montserrat:wght@700&family=Open+Sans:wght@400;700&display=swap', false ); }        
	 add_action( 'wp_enqueue_scripts', 'fwd_add_google_fonts' );
